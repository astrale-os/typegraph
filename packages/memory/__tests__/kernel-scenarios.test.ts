...
describe("Kernel-like Scenarios", () => {
  ...
  describe("Module Hierarchies", () => {
    ...
    it("should move a module to a new parent", async () => {
      ...
      const doc = await graph.mutate.createChild("module", folder1.id, { name: "Doc", role: "item" })
  ...
  describe("Type System", () => {
    it("should assign types to modules", async () => {
      ...
      const doc = await graph.mutate.createChild("module", folder.id, { name: "Notes.md", role: "item" })
  ...
  describe("Permissions", () => {
    ...
    it("should handle multiple permissions to different modules", async () => {
      ...
      await graph.mutate.link("hasPerm", avatar.id, doc1.id, { effect: "allow", read: true, edit: true })
      await graph.mutate.link("hasPerm", avatar.id, doc2.id, { effect: "allow", read: true, edit: false })
      await graph.mutate.link("hasPerm", avatar.id, doc3.id, { effect: "deny", read: false, edit: false })
  ...
  describe("Spaces and Avatars", () => {
    it("should create a space with multiple avatars", async () => {
      ...
      // Create avatars
      const alice = await graph.mutate.create("avatar", { email: "alice@acme.com", displayName: "Alice" })
      const bob = await graph.mutate.create("avatar", { email: "bob@acme.com", displayName: "Bob" })
      const charlie = await graph.mutate.create("avatar", { email: "charlie@acme.com", displayName: "Charlie" })
  ...
  describe("Complex Graph Structures", () => {
    it("should handle a realistic workspace hierarchy", async () => {
      ...
      const docsFolder = await graph.mutate.createChild("module", projectA.id, { name: "Documents", role: "container" })
      const doc1 = await graph.mutate.createChild("module", docsFolder.id, { name: "Architecture.md", role: "item" })
      const doc2 = await graph.mutate.createChild("module", docsFolder.id, { name: "API.md", role: "item" })
      ...
      const tasksFolder = await graph.mutate.createChild("module", projectB.id, { name: "Tasks", role: "container" })
      const task1 = await graph.mutate.createChild("module", tasksFolder.id, {
        name: "Implement feature X",
        role: "item",
      })
      const task2 = await graph.mutate.createChild("module", tasksFolder.id, { name: "Fix bug Y", role: "item" })
    ...
    it("should handle a multi-tenant scenario", async () => {
      ...
      const acmeRoot = await graph.mutate.create("module", { name: "Acme Files", role: "container" })
      const globexRoot = await graph.mutate.create("module", { name: "Globex Files", role: "container" })

      // Permissions - each user can only access their space's modules
      await graph.mutate.link("hasPerm", acmeUser.id, acmeRoot.id, { effect: "allow", read: true, edit: true })
      await graph.mutate.link("hasPerm", globexUser.id, globexRoot.id, { effect: "allow", read: true, edit: true })
  ...
  describe("Export and Import", () => {
    it("should export and import a complete graph", async () => {
      ...
      const doc = await graph.mutate.createChild("module", folder.id, { name: "File.md", role: "item" })
