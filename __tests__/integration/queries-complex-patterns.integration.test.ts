...
describe('Complex Query Patterns', () => {
  ...
  it('optional traversal returning null', async () => {
    ...
    // Note: hasParent has outbound: 'optional' cardinality, so .to() returns
    // OptionalNodeBuilder whose execute() returns T | null, not T[]
  ...
  it('optional traversal returning value', async () => {
    ...
    const parents = await ctx.graph.raw<{ id: string }>(
  ...
  it('complex WHERE with AND/OR/NOT', async () => {
    ...
    // Note: startsWith is not implemented in WhereBuilder, using eq for name check
    ...
    // Should include Alice (active) and Bob (not inactive+Charlie)
    // Should exclude Charlie (inactive AND name is Charlie)
