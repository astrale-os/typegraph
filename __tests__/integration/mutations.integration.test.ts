 ...
 *
 * Tests mutation execution against a real database instance.
 *
 * Note: FalkorDB does not support ACID transactions with rollback.
 * Tests requiring transactions are skipped when running against FalkorDB.
 */
...
// FalkorDB doesn't support ACID transactions - skip transaction tests
...
describe('Mutation Integration Tests', () => {
  ...
  describe('Edge CRUD', () => {
    ...
    it('creates an edge without properties', async () => {
      ...
      const result = await ctx.graph.mutate.link(
        'likes',
        ctx.data.users.bob,
        ctx.data.posts.graphql,
      )
    ...
    it('removes an edge (unlink)', async () => {
      ...
      const result = await ctx.graph.mutate.unlink(
        'likes',
        ctx.data.users.alice,
        ctx.data.posts.graphql,
      )
  ...
  describe('Hierarchy Operations', () => {
    ...
    it('moves a node to new parent', async () => {
      ...
      const result = await ctx.graph.mutate.move(ctx.data.folders.work, ctx.data.folders.root, {
        edge: 'hasParent',
      })
  ...
  describe('Transactions', () => {
    it.skipIf(isFalkorDB)('commits successful transaction', async () => {
    ...
    it.skipIf(isFalkorDB)('rolls back failed transaction', async () => {
      try {
        await ctx.graph.mutate.transaction(async (tx) => {
  ...
  describe('Upsert Operations', () => {
    ...
    it.skipIf(isFalkorDB)('works in transactions', async () => {
      await ctx.graph.mutate.transaction(async (tx) => {
