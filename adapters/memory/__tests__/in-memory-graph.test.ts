...
describe('createInMemoryGraph', () => {
  ...
  describe('whereConnectedTo queries', () => {
    it('should filter by outgoing edge connection', async () => {
      ...
      const parent = await graph.mutate.create('post', {
        title: 'Parent',
        content: 'Parent content',
      })

      // Create children that have hasParent edge pointing to parent
      const child1 = await graph.mutate.createChild('post', parent.id, {
        title: 'Child 1',
        content: 'C1',
      })
      const child2 = await graph.mutate.createChild('post', parent.id, {
        title: 'Child 2',
        content: 'C2',
      })
      ...
      const childrenOfParent = await graph
        .node('post')
        .whereConnectedTo('hasParent', parent.id)
        .execute()
    ...
    it('should filter by multiple whereConnectedTo conditions', async () => {
      ...
      const child1 = await graph.mutate.createChild('post', parentA.id, {
        title: 'Child 1',
        content: 'C1',
      })
      const child2 = await graph.mutate.createChild('post', parentA.id, {
        title: 'Child 2',
        content: 'C2',
      })
      const child3 = await graph.mutate.createChild('post', parentB.id, {
        title: 'Child 3',
        content: 'C3',
      })

      // Query children under parentA only
      const childrenOfA = await graph
        .node('post')
        .whereConnectedTo('hasParent', parentA.id)
        .execute()
      ...
      const childrenOfB = await graph
        .node('post')
        .whereConnectedTo('hasParent', parentB.id)
        .execute()
    ...
    it('should return empty array when no matching connections exist', async () => {
      ...
      const children = await graph
        .node('post')
        .whereConnectedTo('hasParent', potentialParent.id)
        .execute()
